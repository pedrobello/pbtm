...
prePlotPBTMModel <- function (Data, ModelResults)
{
  ...
  p <- ggplot(data=TreatData, aes(x=Time, y=Germ,color=TreatFactor1, alpha = TreatFactor2)) + geom_point(shape=19, size=2) + xlab("Time") + ylab("Cumulative (%)") +
    ...
    annotate("text", x = -Inf, y = Inf, label = paste("Model Parameters"), color = "grey0", hjust = -0.1, vjust = 1) +
    annotate("text", x = -Inf, y = Inf, label = paste(ModPar1Label, ModPar1), color = "grey0", hjust = -0.1, vjust = 2.5, parse = TRUE) +
    annotate("text", x = -Inf, y = Inf, label = paste(ModPar2Label, ModPar2), color = "grey0", hjust = -0.1, vjust = 3.5, parse = TRUE) +
    annotate("text", x = -Inf, y = Inf, label = paste(ModPar3Label, ModPar3), color = "grey0", hjust = -0.1, vjust = 4.5, parse = TRUE) +
    annotate("text", x = -Inf, y = Inf, label = paste(ModPar4Label, ModPar4), color = "grey0", hjust = -0.1, vjust = 5.5, parse = TRUE) +
    annotate("text", x = -Inf, y = Inf, label = paste(ModPar5Label, ModPar5), color = "grey0", hjust = -0.1, vjust = 6.5, parse = TRUE)
  p
